...
class Console{
	...
	static int VerifyInit(int InitStatus){
		const char* Message = "";
		switch(InitStatus)
		{
			case 0:
			...
			break;
			case 1:
				Message = "Console Failed to Initialize";
			break;
		}
		printf("Console Init Result:\n	%s.\n", Message);
		return InitStatus;
	}
    static int init(){
        #ifdef _WIN32
...
int Console::statusCode = Console::VerifyInit(Console::init());
