...
int main() {
  try {
    ...
    // auto major = gladGLversionStruct();
    ...
    auto MVPMatrixUniform =
        glGetUniformLocation(shader.ProgrammID, "MVPMatrix");
    ...
    uint diffuseUniform =
        glGetUniformLocation(shader.ProgrammID, "DiffuseTexture");
    uint normalUniform =
        glGetUniformLocation(shader.ProgrammID, "NormalTexture");
    ...
    while (glfwWindowShouldClose(window) == false &&
      ...
      if (Keyboard::GetKeyDown(KeyCode::ENTER)) {
        glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
      }
      ...
      if (Keyboard::GetKeyDown(KeyCode::F5)) shader.Reload();
      ...
      prim.transform.Rotation =
          Quaternion::euler(0, Time::TotalTime / Math::PI(), 0);

      float Time = fmodf(Time::TotalTime * 100.0, 360.0f) / 360.0f;
      dirLights[0].color = HSLtoRGB(Time, 1.0f, .5f);
      ptLights[0].position = Vector3(sinf(Time::TotalTime * 2) * 5.0f, 0.0f,
                                     cosf(Time::TotalTime * 2) * 5.0f);
